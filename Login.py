...
db = mysql.connector.connect(
    host="localhost",
    user="root",
    password="Noy@12223390",  # Replace with your MySQL password
...
def login_user():
    ...
    if result and bcrypt.checkpw(password.encode('utf-8'), result[0].encode('utf-8')):
        ...
        show_main_window()  # Show the main application window after successful login
    else:
...
def show_main_window():
    ...
    def send():
        if textReceipt.get(1.0,END)=='\n':
            ...
            root2.config(bg='#8B5A2B')
            root2.geometry('485x620+1+1')

            logoImage=PhotoImage(file='image/sender.png')
            label=Label(root2,image=logoImage,bg='#8B5A2B')
            label.pack(pady=5)

            numberLabel=Label(root2,text='Mobile Number',font=('arial',10,'bold underline'),bg='#8B5A2B',fg='white')
            numberLabel.pack(pady=5)

            numberfield=Entry(root2,font=('helvetica',22,'bold'),bd=3,width=24)
            numberfield.pack(pady=5)

            billLabel = Label(root2, text='Bill Details', font=('arial', 10, 'bold underline'), bg='#8B5A2B', fg='white')
            ...
            textarea.insert(END, f'Service Tax\t\t\t{1}$\n')
            textarea.insert(END, f'Total Cost\t\t\t{subtotalofItems + 1}$\n')

            sendButton=Button(root2,text='SEND',font=('arial',19,'bold'),bg='white',fg='#8B5A2B',bd=7,relief=GROOVE
                            ,command=send_msg)
    ...
    def receipt():
        ...
        if costoffoodvar.get() != '' or costofcakesvar.get() != '' or costofdrinksvar.get() != '':
            ...
            textReceipt.insert(END,'***************************************************************\n')
            if e_roti.get()!='0':
                textReceipt.insert(END,f'Matcha Salted Fries\t\t\t{int(e_roti.get())*4.00}\n\n')

            if e_daal.get()!='0':
                textReceipt.insert(END,f'Matcha Soba Noodles\t\t\t{int(e_daal.get())*7.00}\n\n')

            if e_fish.get()!='0':
                textReceipt.insert(END,f'Matcha-Crusted Fried Chicken\t\t\t{int(e_fish.get())*8.00}\n\n')

            if e_chawal.get() != '0':
                textReceipt.insert(END, f'Matcha Rice Bowls:\t\t\t{int(e_chawal.get()) * 9.00}\n\n')

            if e_sabji.get() != '0':
                textReceipt.insert(END, f'Matcha Sushi Rows:\t\t\t{int(e_sabji.get()) * 10.00}\n\n')

            if e_paneer.get() != '0':
                textReceipt.insert(END, f'Matcha Popcorn:\t\t\t{int(e_paneer.get()) * 5.00}\n\n')

            if e_kebab.get() != '0':
                textReceipt.insert(END, f'Matcha Almonds:\t\t\t{int(e_kebab.get()) * 4.10}\n\n')

            if e_chicken.get() != '0':
                textReceipt.insert(END, f'Paneer:\t\t\t{int(e_chicken.get()) * 6.10}\n\n')

            if e_mutton.get() != '0':
                textReceipt.insert(END, f'Chicken:\t\t\t{int(e_mutton.get()) * 5.00}\n\n')

            if e_lassi.get() != '0':
                textReceipt.insert(END, f'Matcha Latte:\t\t\t{int(e_lassi.get()) * 5.00}\n\n')

            if e_coffee.get() != '0':
                textReceipt.insert(END, f'Iced Matcha Lemonade:\t\t\t{int(e_coffee.get()) * 4.00}\n\n')

            if e_faluda.get() != '0':
                textReceipt.insert(END, f'Matcha Bubble Tea:\t\t\t{int(e_faluda.get()) * 6.10}\n\n')

            if e_shikanji.get() != '0':
                textReceipt.insert(END, f'Matcha Affogato:\t\t\t{int(e_shikanji.get()) * 4.00}\n\n')

            if e_jaljeera.get() != '0':
                textReceipt.insert(END, f'Matcha Mojito:\t\t\t{int(e_jaljeera.get()) * 3.34}\n\n')

            if e_roohafza.get() != '0':
                textReceipt.insert(END, f'Coconut Matcha:\t\t\t{int(e_roohafza.get()) * 7.00}\n\n')

            if e_masalatea.get() != '0':
                textReceipt.insert(END, f'Masala Tea:\t\t\t{int(e_masalatea.get()) * 4.25}\n\n')

            if e_badammilk.get() != '0':
                textReceipt.insert(END, f'Badam Milk:\t\t\t{int(e_badammilk.get()) * 3.75}\n\n')

            if e_coldrinks.get() != '0':
                textReceipt.insert(END, f'Cold Drinks:\t\t\t{int(e_coldrinks.get()) * 5.75}\n\n')

            if e_oreo.get() != '0':
                textReceipt.insert(END, f'Matcha Cheese Cake:\t\t\t{int(e_oreo.get()) * 5.99}\n\n')

            if e_apple.get() != '0':
                textReceipt.insert(END, f'Matcha Tiramisu:\t\t\t{int(e_apple.get()) * 4.99}\n\n')

            if e_kitkat.get() != '0':
                textReceipt.insert(END, f'Matcha Ice Cream:\t\t\t{int(e_kitkat.get()) * 5.67}\n\n')

            if e_banana.get() != '0':
                textReceipt.insert(END, f'Matcha Swiss Roll:\t\t\t{int(e_banana.get()) * 3.1}\n\n')

            if e_brownie.get() != '0':
                textReceipt.insert(END, f'Matcha Cookies:\t\t\t{int(e_brownie.get()) * 3.00}\n\n')

            if e_pineapple.get() != '0':
                textReceipt.insert(END, f'Matcha Donuts:\t\t\t{int(e_pineapple.get()) * 3.00}\n\n')

            if e_chocolate.get() != '0':
                textReceipt.insert(END, f'Matcha Pancake:\t\t\t{int(e_chocolate.get()) * 2.1}\n\n')

            if e_blackforest.get() != '0':
                textReceipt.insert(END, f'Chololate Matcha:\t\t\t{int(e_blackforest.get()) * 2.1}\n\n')

            if e_vanilla.get() != '0':
                textReceipt.insert(END, f'KitKat:\t\t\t{int(e_vanilla.get()) * 4.25}\n\n')
            ...
            textReceipt.insert(END, f'Service Tax\t\t\t{1}$\n\n')
            textReceipt.insert(END, f'Total Cost\t\t\t{subtotalofItems+1}$\n\n')
            textReceipt.insert(END,'***************************************************************\n')
    ...
    def totalcost():
        ...
        if var1.get() != 0 or var2.get() != 0 or var3.get() != 0 or var4.get() != 0 or var5.get() != 0 or \
            ...
            priceofFood=(item1*4.00)+(item2*7.00)+(item3*8.00)+(item4*9.00)+ (item5*10.00) + (item6 * 5.00) + (item7 * 8.00) \
                        + (item8 * 4.1) + (item9 * 6.1)

            priceofDrinks=(item10*5.00)+(item11*4.1)+ (item12 * 4.00) + (item13 * 6.1) + (item14 * 4.00) + (item15 * 3.34) \
                        + (item16 * 7.00) + (item17 * 4.25) + (item18 * 3.75)

            priceofCakes=(item19*5.75)+(item20*5.99)+ (item21 * 4.99) + (item22 * 5.67) + (item23 * 3.1) + (item24 * 3.00) \
                        + (item25 * 3.00) + (item26 * 2.1) + (item27 * 4.25)
    ...
    root.config(bg='#8B5A2B')

    topFrame=Frame(root,bd=10,relief=RIDGE,bg='#8B5A2B')
    topFrame.pack(side=TOP)

    labelTitle=Label(topFrame,text='Matcha Essence Ventures Cafe',font=('arial',30,'bold'),fg='#8B5A2B',bd=9,
                    bg='#487D49',width=51)
    labelTitle.grid(row=0,column=0)

    #frames

    menuFrame=Frame(root,bd=10,relief=RIDGE,bg='#8B5A2B')
    menuFrame.pack(side=LEFT)

    costFrame=Frame(menuFrame,bd=4,relief=RIDGE,bg='#8B5A2B',pady=10)
    costFrame.pack(side=BOTTOM)

    foodFrame=LabelFrame(menuFrame,text='Foods',font=('arial',19,'bold'),bd=10,relief=RIDGE,fg='#487D49',)
    foodFrame.pack(side=LEFT)

    drinksFrame=LabelFrame(menuFrame,text='Drinks',font=('arial',19,'bold'),bd=10,relief=RIDGE,fg='#487D49')
    drinksFrame.pack(side=LEFT)

    cakesFrame=LabelFrame(menuFrame,text='Desserts',font=('arial',19,'bold'),bd=10,relief=RIDGE,fg='#487D49')
    cakesFrame.pack(side=LEFT)

    rightFrame=Frame(root,bd=15,relief=RIDGE,bg='#487D49')
    rightFrame.pack(side=RIGHT)

    calculatorFrame=Frame(rightFrame,bd=1,relief=RIDGE,bg='#487D49')
    calculatorFrame.pack()

    recieptFrame=Frame(rightFrame,bd=4,relief=RIDGE,bg='#487D49')
    recieptFrame.pack()

    buttonFrame=Frame(rightFrame,bd=3,relief=RIDGE,bg='#487D49')
    buttonFrame.pack()
    ...
    roti=Checkbutton(foodFrame,text='Matcha Salted Fries',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var1
                    ,command=roti)
    roti.grid(row=0,column=0,sticky=W)

    daal=Checkbutton(foodFrame,text='Matcha Soba Noodles',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var2
                    ,command=daal)
    daal.grid(row=1,column=0,sticky=W)

    fish=Checkbutton(foodFrame,text='Matcha-Crusted Fried Chicken',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var3
                    ,command=fish)
    fish.grid(row=2,column=0,sticky=W)

    sabji=Checkbutton(foodFrame,text='Matcha Rice Bowls',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var4
                    ,command=sabji)
    sabji.grid(row=3,column=0,sticky=W)

    kebab=Checkbutton(foodFrame,text='Matcha Sushi Rolls',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var5
                    ,command=kebab)
    kebab.grid(row=4,column=0,sticky=W)

    chawal=Checkbutton(foodFrame,text='Matcha Popcorn',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var6
                    ,command=chawal)
    chawal.grid(row=5,column=0,sticky=W)

    mutton=Checkbutton(foodFrame,text='Matcha Almonds',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var7,
                    command=mutton)
    mutton.grid(row=6,column=0,sticky=W)

    paneer=Checkbutton(foodFrame,text='Paneer',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var8
                    ,command=paneer)
    paneer.grid(row=7,column=0,sticky=W)

    chicken=Checkbutton(foodFrame,text='Chicken',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var9
                        ,command=chicken)
    ...
    lassi=Checkbutton(drinksFrame,text='Matcha Latte',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var10
                    ,command=lassi)
    lassi.grid(row=0,column=0,sticky=W)

    coffee=Checkbutton(drinksFrame,text='Iced Matcha Lemonade',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var11
                    ,command=coffee)
    coffee.grid(row=1,column=0,sticky=W)

    faluda=Checkbutton(drinksFrame,text='Matcha Bubble Tea',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var12
                    ,command=faluda)
    faluda.grid(row=2,column=0,sticky=W)

    shikanji=Checkbutton(drinksFrame,text='Matcha Affogato',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var13
                        ,command=shikanji)
    shikanji.grid(row=3,column=0,sticky=W)

    jaljeera=Checkbutton(drinksFrame,text='Matcha Mojito',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var14
                        ,command=jaljeera)
    jaljeera.grid(row=4,column=0,sticky=W)

    roohafza=Checkbutton(drinksFrame,text='Coconut Matcha',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var15
                        ,command=roohafza)
    roohafza.grid(row=5,column=0,sticky=W)

    masalatea=Checkbutton(drinksFrame,text='Masala Tea',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var16
                        ,command=masalatea)
    masalatea.grid(row=6,column=0,sticky=W)

    badammilk=Checkbutton(drinksFrame,text='Badam Milk',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var17
                        ,command=badammilk)
    badammilk.grid(row=7,column=0,sticky=W)

    colddrinks=Checkbutton(drinksFrame,text='Cold Drinks',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var18
                        ,command=colddrinks)
    ...
    textcolddrinks = Entry(drinksFrame, font=('arial', 10, 'bold'), bd=7, width=6, state=DISABLED, textvariable=e_coldrinks)
    textcolddrinks.grid(row=8, column=1)

    #Cakes

    oreocake=Checkbutton(cakesFrame,text='Matcha Cheese Cake',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var19
                        ,command=oreo)
    oreocake.grid(row=0,column=0,sticky=W)

    applecake=Checkbutton(cakesFrame,text='Matcha Tiramisu',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var20
                        ,command=apple)
    applecake.grid(row=1,column=0,sticky=W)

    kitkatcake=Checkbutton(cakesFrame,text='Matcha Ice Cream',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var21
                        ,command=kitkat)
    kitkatcake.grid(row=2,column=0,sticky=W)

    vanillacake=Checkbutton(cakesFrame,text='Matcha Swiss Roll',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var22
                            ,command=vanilla)
    vanillacake.grid(row=3,column=0,sticky=W)

    bananacake=Checkbutton(cakesFrame,text='Matcha Cookies ',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var23
                        ,command=banana)
    bananacake.grid(row=4,column=0,sticky=W)

    browniecake=Checkbutton(cakesFrame,text='Matcha Donuts',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var24
                            ,command=brownie)
    browniecake.grid(row=5,column=0,sticky=W)

    pineapplecake=Checkbutton(cakesFrame,text='Matcha Pancake',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var25
                            ,command=pineapple)
    pineapplecake.grid(row=6,column=0,sticky=W)

    chocolatecake=Checkbutton(cakesFrame,text='Chocolate Matcha',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var26
                            ,command=chocolate)
    chocolatecake.grid(row=7,column=0,sticky=W)

    blackforestcake=Checkbutton(cakesFrame,text='Kitkat',font=('arial',10,'bold'),onvalue=1,offvalue=0,variable=var27
                                ,command=blackforest)
    ...
    labelCostofFood=Label(costFrame,text='Cost of Foods:',font=('arial',16,'bold'),bg='#8B5A2B',fg='white')
    ...
    labelCostofDrinks=Label(costFrame,text='Cost of Drinks:',font=('arial',16,'bold'),bg='#8B5A2B',fg='white')
    ...
    labelCostofCakes=Label(costFrame,text='Cost of Cakes:',font=('arial',16,'bold'),bg='#8B5A2B',fg='white')
    ...
    labelSubTotal=Label(costFrame,text='Sub Total:',font=('arial',16,'bold'),bg='#8B5A2B',fg='white')
    ...
    labelServiceTax=Label(costFrame,text='Service Tax:',font=('arial',16,'bold'),bg='#8B5A2B',fg='white')
    ...
    labelTotalCost=Label(costFrame,text='Total Cost:',font=('arial',16,'bold'),bg='#8B5A2B',fg='white')
    ...
    buttonTotal=Button(buttonFrame,text='Total',font=('arial',14,'bold'),fg='white',bg='#487D49',bd=3,padx=5,
                    command=totalcost)
    buttonTotal.grid(row=0,column=0)

    buttonReceipt=Button(buttonFrame,text='Receipt',font=('arial',14,'bold'),fg='white',bg='#487D49',bd=3,padx=5
                        ,command=receipt)
    buttonReceipt.grid(row=0,column=1)

    buttonSave=Button(buttonFrame,text='Save',font=('arial',14,'bold'),fg='white',bg='#487D49',bd=3,padx=5
                    ,command=save)
    buttonSave.grid(row=0,column=2)

    buttonSend=Button(buttonFrame,text='Send',font=('arial',14,'bold'),fg='white',bg='#487D49',bd=3,padx=5,command=send)
    buttonSend.grid(row=0,column=3)

    buttonReset=Button(buttonFrame,text='Reset',font=('arial',14,'bold'),fg='white',bg='#487D49',bd=3,padx=5,
                    command=reset)
    ...
    def answer():
        ...
        global operator
        try:
            result = eval(operator)  # Evaluate the expression stored in operator
            calculatorField.delete(0, tk.END)
            calculatorField.insert(tk.END, result)
            operator = str(result)  # Store the result in the operator for future calculations
        except (SyntaxError, ZeroDivisionError) as e:
            calculatorField.delete(0, tk.END)
            calculatorField.insert(tk.END, "Error")  # Display error message
            operator = ''  # Clear the operator in case of error
    ...
    button7=Button(calculatorFrame,text='7',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6,
                command=lambda:buttonClick('7'))
    button7.grid(row=1,column=0)

    button8=Button(calculatorFrame,text='8',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6,
                command=lambda:buttonClick('8'))
    button8.grid(row=1,column=1)

    button9=Button(calculatorFrame,text='9',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6
                ,command=lambda:buttonClick('9'))
    button9.grid(row=1,column=2)

    buttonPlus=Button(calculatorFrame,text='+',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6
                    ,command=lambda:buttonClick('+'))
    buttonPlus.grid(row=1,column=3)

    button4=Button(calculatorFrame,text='4',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6
                ,command=lambda:buttonClick('4'))
    button4.grid(row=2,column=0)

    button5=Button(calculatorFrame,text='5',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6
                ,command=lambda:buttonClick('5'))
    button5.grid(row=2,column=1)

    button6=Button(calculatorFrame,text='6',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6
                ,command=lambda:buttonClick('6'))
    button6.grid(row=2,column=2)

    buttonMinus=Button(calculatorFrame,text='-',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6
                    ,command=lambda:buttonClick('-'))
    buttonMinus.grid(row=2,column=3)

    button1=Button(calculatorFrame,text='1',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6
                ,command=lambda:buttonClick('1'))
    button1.grid(row=3,column=0)

    button2=Button(calculatorFrame,text='2',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6
                ,command=lambda:buttonClick('2'))
    button2.grid(row=3,column=1)

    button3=Button(calculatorFrame,text='3',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6
                ,command=lambda:buttonClick('3'))
    button3.grid(row=3,column=2)

    buttonMult=Button(calculatorFrame,text='*',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6
                    ,command=lambda:buttonClick('*'))
    buttonMult.grid(row=3,column=3)

    buttonAns=Button(calculatorFrame,text='=',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6,
                    command=answer)
    buttonAns.grid(row=4,column=0)

    buttonClear=Button(calculatorFrame,text='Clear',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6
                    ,command=clear)
    buttonClear.grid(row=4,column=1)

    button0=Button(calculatorFrame,text='0',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6
                ,command=lambda:buttonClick('0'))
    button0.grid(row=4,column=2)

    buttonDiv=Button(calculatorFrame,text='/',font=('arial',16,'bold'),fg='yellow',bg='#487D49',bd=6,width=6,
                    command=lambda:buttonClick('/'))
...
# Main Tkinter window (login window)
login_window = tk.Tk()
...
tk.Button(login_frame, text="Register", command=switch_to_register).pack()

# Register Frame
register_frame = tk.Frame(login_window)
...
tk.Button(register_frame, text="Back to Login", command=switch_to_login).pack()
